...
namespace Rocket.Unturned.Chat
{
    public sealed class UnturnedChat : MonoBehaviour
    {
        ...
        private void handleChat(SteamPlayer steamPlayer, EChatMode chatMode, ref Color incomingColor, ref bool isRich, string message, ref bool cancel)
        {
            cancel = false;
            ...
            try
            {
                ...
                color = UnturnedPlayerEvents.firePlayerChatted(player, chatMode, player.Color, ref isRich, message, ref cancel);
            }
            catch (Exception ex)
            {
        ...
        public static void Say(CSteamID CSteamID, string message, Color color, bool rich)
        {
            ...
            else
            {
                foreach (string m in wrapMessage(message))
                {
                    ChatManager.instance.channel.send("tellChat", CSteamID, ESteamPacket.UPDATE_UNRELIABLE_BUFFER, new object[] { CSteamID.Nil, (byte)EChatMode.SAY, color, rich, m });
                }
            }
        }
